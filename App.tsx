...
const App: React.FC = () => {
  ...
  // Load bank from localStorage on mount
  useEffect(() => {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      // Para el test inmediato tras subir, usamos las del archivo pero desordenadas
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          >
            ...
            <h1 className="font-bold text-slate-900 hidden sm:block">IntelliBank AI</h1>
          </div>
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 py-8">
        {view === AppView.HOME && (
          <div className="space-y-12 py-10">
            <div className="text-center max-w-2xl mx-auto">
              <h2 className="text-4xl font-black text-slate-900 mb-4 tracking-tight">Tu asistente de evaluación inteligente.</h2>
              <p className="text-lg text-slate-600">Sube un Word para extraer preguntas automáticamente o gestiona tu banco de datos local para ahorrar tokens.</p>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-8">
              ...
              <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200 flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-slate-500 mb-6">Actualmente tienes <strong>{bank.length}</strong> preguntas guardadas. Practicarás con el banco completo desordenado.</p>
                </div>
                <button 
                  onClick={startQuizFromBank}
                  disabled={bank.length === 0}
                  className="w-full py-4 bg-slate-900 text-white rounded-2xl font-bold hover:bg-black transition-all disabled:bg-slate-200 disabled:text-slate-400"
                >
        ...
        {view === AppView.LOADING && (
          <div className="flex flex-col items-center justify-center min-h-[400px]">
            ...
            <p className="mt-6 font-bold text-slate-700">Gemini está analizando y optimizando tu banco...</p>
          </div>
        )}
